...
computeMEM <- function(refPopMEMFile, clustsMEMFile) {
  ...
  for (phengraphCluster in clusts$MEDIAN_Group.1) {
    ...
    clustData = data.frame(PHENOGRAPH_CLUSTER=phengraphCluster)
    for (marker in markers) {
      ...
      colnames(tmp)=paste0(marker,"_MEM_RAWz_BASE")
      ...
      colnames(tmp)=paste0(marker,"_MEM_RAW_BASE_MINUS_CLUST")
