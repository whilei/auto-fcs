...
# clustTable=as.data.frame(table(as.character(currentData$SAMPLE)))
g=ggplot(data=sub, aes(sub$TOTAL_PHENOGRAPH_COUNTS)) + geom_histogram(binwidth=100)+xlab("number of events in cytotoxic phenograph clusters")+ylab("number of clusters")
...
# clustTable=as.data.frame(table(as.character(currentData$SAMPLE)))
g=ggplot(data=merge, aes(merge$FREQ_TOTAL)) + geom_histogram(binwidth=.0001)+xlab("number of events in cytotoxic phenograph clusters/total events in fcs")+ylab("number of clusters")
