...
# use=bn %in% ctlCounts$FCS.ID
 ...
 for(file in centFiles){
   if(gsub("_subFirst_TRUE_normalize_FALSE.cents.scale","",basename(file)) %in% ctlCounts$FCS.ID){
...
- running phenograph on Cytotoxic T cells from non-"manual" samples
  - starting within **CtlA fcs files only**
...
```{r }
ctlA=centsCtls[which(centsCtls$CTL=="A"),]
clustTable=as.data.frame(table(as.character(ctlA$SAMPLE)))
...
- n = `r length(unique(ctlA$SAMPLE))` fcs files
...
- "x"-axis are individual phenograph clusters ordered by CtlA_FCS1,CtlA_FCS2,....
...
for (mark in markersToCluster) {
      ctlA[, mark] = squish(ctlA[, mark], range = c(min, max))
}
superheat(
    as.matrix(t(ctlA[, markersToCluster])),
