...
combo = merge(summaries,interestPercentsM,by.x = "KEY",by.y = "KEY")

pops=unique(combo$POPULATION)

for(pop in pops){
  plotData=combo[which(combo$POPULATION==pop),]
  cor1 =cor.test(as.numeric(plotData$PARENT_FREQ),as.numeric(plotData$value),method = "pearson",na.action="na.omit")
  cor2=cor.test(as.numeric(plotData$PARENT_FREQ),as.numeric(plotData$value),method = "spearman",na.action="na.omit")
