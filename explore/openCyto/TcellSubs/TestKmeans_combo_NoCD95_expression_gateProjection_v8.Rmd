...
```{r func, eval=TRUE ,echo=FALSE,warning=FALSE,fig.height=40,fig.width=8,message=FALSE}
require(flowCore)
require(cytofkit)
library(ggcyto)
library(ClusterR)
...
for (samp in mapper$FCS[6:6]) {
  ...
  for (method in methodsClust) {
   ...
   polyTest=  ggplot(results,
      ...
      theme(legend.position = "bottom") + xlim(0, 225) +
      geom_density2d() + guides(colour = guide_legend(override.aes = list(alpha = 1))) 
   ...
   for (pop in unique(results$CURRENT_COLOR)) {
    ...
    p2c = ggplot(results,
                 aes(x = CCR7,
                     y = CD45,
                     colour = CURRENT_COLOR)) +
      geom_point(alpha = alpha) +ylim(0,225)+
      xlab(paste0("CCR7")) +
      ylab(paste0("CD45")) + t2 + ggtitle(paste0(method, "\n", samp)) +
      theme(legend.position = "bottom") + facet_wrap(~ SET) + xlim(0, 225) +
      geom_density2d() + guides(colour = guide_legend(override.aes = list(alpha = 1)))
    ...
    ggplot(results,
                 aes(x = CCR7,
                     y = CD45,
                     colour = CURRENT_COLOR)) +
      geom_point(alpha = .5) +ylim(0,225)+
      xlab(paste0("CCR7")) +
      ylab(paste0("CD45")) + t2 + ggtitle(paste0(method, "\n", samp)) +
      theme(legend.position = "bottom") + facet_wrap(~ SET) + xlim(0, 225) + guides(colour = guide_legend(override.aes = list(alpha = 1)))
     ggplot(results[which(results$CURRENT_COLOR=="effector"),],
                 aes(x = CCR7,
                     y = CD45)) +
      geom_point(alpha = .5,color="darkgreen") +ylim(0,225)+
      xlab(paste0("CCR7")) +
      ylab(paste0("CD45")) + t2 + ggtitle(paste0(method, "\n", samp)) +
      theme(legend.position = "bottom") + facet_wrap(~ SET) + xlim(0, 225) + guides(colour = guide_legend(override.aes = list(alpha = 1)))
     ...
     ggplot(results[which(results$CURRENT_COLOR=="effector"),],
                 aes(x = CD95,
                     y = CD28 )) +
      geom_point(alpha = .5,colour = "darkgreen") +ylim(0,225)+
      xlab(paste0("CD95")) +
      ylab(paste0("CD28")) + t2 + ggtitle(paste0(method, "\n", samp)) +
      theme(legend.position = "bottom") + facet_wrap(~ SET) + xlim(0, 225) + guides(colour = guide_legend(override.aes = list(alpha = 1)))
