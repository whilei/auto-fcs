...
summFiles <-
  list.files(inDir,
             ...
             full = TRUE,
             recursive = FALSE)
details = file.info(list.files(inDir,
             pattern = "counts.txt$",
             full = TRUE,
             recursive = FALSE))
details = details[with(details, order(as.POSIXct(mtime))), ]
details$date=as.Date(as.POSIXct(details$mtime))
details=details[which(details$date< as.Date('2018-02-24')),]
summFiles=rownames(details)
...
write.table( summFiles,file ="./filesUse.txt",sep = "\t",quote = FALSE,row.names = FALSE)
...
pops = unique(comboCtls$POPULATION)
  ...
  plotIt <- function(plotData,compVar,color) {
    ...
    PEARSON_CONF_INT = paste0(cor1$conf.int),
    PEARSON_PVALUE = cor1$p.value,
    PEARSON_FORMAT = paste0(round(cor1$estimate, digits = 2),
                            " (",
                            paste0(round(cor1$conf.int, digits = 2), collapse = ", "),
                            ")"), 
...
for (pop in pops) {
  ...
  g1 = ggplot(plotData,
              aes(x = PARENT_FREQ,
                  y = value, color = MANUAL_ANNOTATION)) +
    geom_point() +
  ...
  tmp = data.frame(
    ...
    METRIC = "PARENT_FREQ",
    PEARSON = cor1$estimate,
    PEARSON_CONF_INT = paste0(cor1$conf.int,collapse = ","),
    PEARSON_PVALUE = cor1$p.value,
    PEARSON_FORMAT = paste0(round(cor1$estimate, digits = 2),
                            " (",
                            paste0(round(cor1$conf.int, digits = 2), collapse = ", "),
...
write.table(
  plotData[, c("SAMPLE", "LAB_ID")],
  file = "./SamplesUsedKmeansTcelSubCor.txt",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
  )
...
# siq=corSummaryAll$PEARSON_PVALUE<0.05
# corSummaryAll$PEARSON_FORMAT[siq]=paste0(corSummaryAll$PEARSON_FORMAT[siq],"*")
# corSummaryAll$PEARSON_FORMAT=paste0(corSummaryAll$PEARSON_FORMAT," (p=",corSummaryAll$PEARSON_PVALUE,")")
print(corSummaryFile)
write.table(
  corSummaryAll,
  file = corSummaryFile,
  sep = "\t",
