...
for (samp in mapper$FCS[1:12]) {
  ...
  if (!file.exists(fileSave)||reCluster) {
    ...
    save(clust,file =paste0(fileSave,".clust") )
  ...
  results$CLUSTER_RAW = km_rc$clusters
    ...
    map=as.data.frame(table(results$KMEANS_CLUSTER,results$POPULATION))
    popsToDump =unique(results$POPULATION)
    for(popToDump in popsToDump ){
      ...
      tmp[,popToDump] =FALSE
      # def =  results$POPULATION %in% c(popToDump)
