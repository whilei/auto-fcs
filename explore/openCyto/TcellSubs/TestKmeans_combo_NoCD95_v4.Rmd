...
```{r func, eval=TRUE ,echo=FALSE,warning=FALSE,fig.height=8,fig.width=8,message=FALSE}
...
for (samp in mapper$FCS[1:15]) {
  ...
  for (method in methodsClust) {
    ...
    p1c = ggplot(results,
      ...
      theme(legend.position = "bottom") + facet_wrap(~ SET)+xlim(0, 250)
  ...
  grid.arrange(p1c, p2c,
               ...
               ncol = 1)
  grid.rect(width = 1.0, height = 1.0, gp = gpar(lwd = 2, col = "black", fill = NA))
