---
title: "Tcell subs - larger manual correl test"
author: "JL"
date: "2/13/2018"
output: 
  html_document: 
...
```{r func, eval=TRUE ,echo=FALSE,warning=FALSE,fig.height=4,fig.width=10,message=FALSE}
require(flowCore)
require(cytofkit)
library(ggcyto)
library(ClusterR)
...
sampleNum=0
# [c(6:7,10)]
fullCounts=paste0(outputDir,"Full.counts.All.txt")
if (!file.exists(fullCounts)){
for (samp in mapper$FCS) {
 ...
 summary = data.frame(SAMPLE=character(),POPULATION=character(),METHOD=character(),COUNT=numeric(),OPT_K_95_28=numeric(),DIFF_K_3V2_95_28=numeric(),NUM_POPS =numeric())
 ...
 for(popula in unique(results$POPULATION)){
   for( parent in parents){
     ...
     numPop=length(unique(results$POPULATION))
     print(paste0("numPops=",numPop))
      tmp = data.frame(
        SAMPLE = samp,
...
write.table( summaries,file = fullCounts,sep = "\t",quote = FALSE,row.names = FALSE)
...
for(pop in unique(summariesMerge$POPULATION.x)){
  plotData =summariesMerge[which(summariesMerge$POPULATION.x==pop&summariesMerge$NUM_POPS.x==4),]
  ...
  tmpCor = data.frame(
    ...
    N = length(plotData$KEY),
    METRIC = "COUNT"
    
    )
  corSummary=rbind(corSummary,tmpCor)
  colorVar = plotData$OPT_K_95_28.x
  pg1= ggplot(plotData,
                 aes(x = COUNT.x,
                     y = COUNT.y,
                     colour = colorVar)) +
...
write.table( corSummary,file = paste0(fullCounts,".cor.txt"),sep = "\t",quote = FALSE,row.names = FALSE)
  
library(knitr)

kable(corSummary,format="markdown")
