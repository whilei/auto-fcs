  ...
  for (method in methodsClust) {
    ...
    for(marker in markers){
      tmp =data.frame(
        ...
        markerValue = scale(results[,marker])
      )
      tmp$MARKER=marker
    ...
    pbOrient1= ggplot(tsneOrient[which(tsneOrient$MARKER=="CCR7"|tsneOrient$MARKER=="CD45"),],
                 aes(x = tsne_1,
                     y = tsne_2)) +
      geom_point(aes(colour = markerValue))+ scale_fill_gradientn(colours = myColor)+
      xlab(paste0("tsne1")) +
      ylab(paste0("tsne2")) + t2 + ggtitle(paste0("All data, normalized", "\n", samp)) +
