...
load(paste0(dir, "all.totalCellCounts.metrics.format.RData"))
tc = tc[which(!is.na(tc$DATE)),]
tc = tc[which(!is.na(tc$EXPERIMENTER)),]
tc = tc[which(!is.na(tc$LAB_ID)),]
tc=tc[which(tc$PANEL!="panel_undetermined"&!is.na(tc$DATE)&!is.na(tc$EXPERIMENTER)),]
manual = read.delim(
  "/Volumes/Beta/data/flow/results_r25_22full_SS_SubCD8_SCD14_Manuals_partialTest/manual",
  stringsAsFactors = FALSE,
  header = FALSE
  )
  use = !(tc$FILE %in% manual$V1)
  tc = tc[use, ]
tc$PANEL_MACHINE=paste0(tc$PANEL,"_",tc$MACHINE)
tc$PANEL_T=paste0(tc$PANEL,"_")
